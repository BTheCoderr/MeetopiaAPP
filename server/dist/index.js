...
const assessNetworkQuality = (socket) => {
    const latency = socket.handshake.query.latency ? parseInt(socket.handshake.query.latency) : 0;
    if (latency < 100)
        return 'good';
    if (latency < 300)
        return 'fair';
...
const cleanupConnection = (socketId, reason) => {
...
io.on('connection', (socket) => {
    ...
    socket.on('ping', (data, callback) => {
        ...
        const pongResponse = {
            timestamp: Date.now(),
            connectionQuality: (connection === null || connection === void 0 ? void 0 : connection.networkQuality) || 'unknown',
            serverLoad: {
                activeConnections: connectionPool.active.size,
    ...
    socket.on('find-match', (data, callback) => {
        ...
        if (!connection) {
            ...
            if (callback)
                callback({ status: 'error', message: 'Connection not found' });
            return;
        }
        ...
        if (waitingUsers.size > 0) {
            ...
            if (callback)
                callback({ status: 'matched', matchData });
            // Emit to both users
            ...
            // Send acknowledgment to requesting user
            if (callback)
                callback({ status: 'waiting', ...waitingResponse });
    ...
    socket.on('offer', (offer) => {
        ...
        if (peerId) {
            ...
            logger.info('🎯 Forwarding WebRTC offer', {
                ...
                hasIceCandidates: ((_a = offer.sdp) === null || _a === void 0 ? void 0 : _a.includes('candidate')) || false
            });
    ...
    socket.on('answer', (answer) => {
        ...
        if (peerId) {
            ...
            logger.info('🎯 Forwarding WebRTC answer', {
                ...
                hasIceCandidates: ((_a = answer.sdp) === null || _a === void 0 ? void 0 : _a.includes('candidate')) || false
            });
    ...
    socket.on('ice-candidate', (candidate) => {
        ...
        if (peerId) {
            ...
            logger.debug('🧊 Forwarding ICE candidate', {
                from: socket.id,
                to: peerId,
                candidateType: ((_a = candidate.candidate) === null || _a === void 0 ? void 0 : _a.split(' ')[7]) || 'unknown',
                protocol: ((_b = candidate.candidate) === null || _b === void 0 ? void 0 : _b.includes('udp')) ? 'UDP' :
                    ((_c = candidate.candidate) === null || _c === void 0 ? void 0 : _c.includes('tcp')) ? 'TCP' : 'unknown'
            });
            ...
            if (Math.random() < 0.1) {
                logger.warn('❌ Received ICE candidate but no active connection found', {
                    socketId: socket.id,
                    candidateType: ((_d = candidate.candidate) === null || _d === void 0 ? void 0 : _d.split(' ')[7]) || 'unknown'
                });
            }
        }
    });
    ...
    socket.on('restore-connection', (data) => {
